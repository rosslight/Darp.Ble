...
public sealed class AttHandleValueNtfTests
{
    ...
    public void TryDecode_ShouldBeValid(string hexBytes, ushort handle, string valueHexBytes)
    {
        byte[] bytes = Convert.FromHexString(hexBytes);
        byte[] valueBytes = Convert.FromHexString(valueHexBytes);

        bool success = AttHandleValueNtf.TryDecode(bytes, out AttHandleValueNtf value, out int decoded);
    ...
    {
        byte[] bytes = Convert.FromHexString(hexBytes);
        bool success = AttHandleValueNtf.TryDecode(bytes, out _, out int decoded);
