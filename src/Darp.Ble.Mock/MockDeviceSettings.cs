...
public sealed class MockDeviceSettings
{
    ...
    public Func<TxPowerLevel, Rssi> TxPowerToRssi { get; set; } =
        txPower => CalculateRssi(txPower, 2, 2);
    ...
    public static Rssi CalculateRssi(
    ...
    {
        return (Rssi)
            (sbyte)((double)txPower - (10.0d * Math.Log10(distance) * environmentalFactor));
    }
}
