...
internal sealed class WinGattServerService(
    GattServerPeer peer,
...
) : GattServerService(peer, BleUuid.FromGuid(winService.Uuid, inferType: true), GattServiceType.Undefined, logger)
{
    ...
    private IObservable<GattServerCharacteristic> DiscoverCharacteristic(
        Func<IAsyncOperation<GattCharacteristicsResult>> getServices
    )
    {
        return Observable.Create<GattServerCharacteristic>(
            async (observer, cancellationToken) =>
            {
                DeviceAccessStatus accessStatus = await _winService
                    .RequestAccessAsync()
                    ...
                    .Subscribe(
                        result =>
                        {
                            if (result.Status is not GattCommunicationStatus.Success)
                            {
                                observer.OnError(
                                    new Exception(
                                        $"Could not query new services for device - got result {result.Status} ({result.ProtocolError})"
                                    )
                                );
                                return;
                            }
