...
internal sealed class WinGattServerCharacteristic(
    ...
    : GattServerCharacteristic(
        service,
        gattCharacteristic.AttributeHandle,
        BleUuid.FromGuid(gattCharacteristic.Uuid, inferType: true),
        (GattProperty)gattCharacteristic.CharacteristicProperties,
        logger
    )
{
    ...
    {
        GattWriteResult result = await _gattCharacteristic
            .WriteValueWithResultAsync(bytes.AsBuffer(), GattWriteOption.WriteWithResponse)
        ...
        if (result.Status is GattCommunicationStatus.ProtocolError)
            throw new GattCharacteristicException(
                this,
                $"Could not write because of protocol error {result.ProtocolError}"
            );
    ...
    {
        GattReadResult result = await _gattCharacteristic
    ...
    {
        await _gattCharacteristic.WriteClientCharacteristicConfigurationDescriptorAsync(
