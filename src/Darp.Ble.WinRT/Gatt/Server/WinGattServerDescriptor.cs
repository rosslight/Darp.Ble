...
internal sealed class WinGattServerDescriptor(
    ...
    : GattServerDescriptor(
        characteristic,
        BleUuid.FromGuid(winDescriptor.Uuid, inferType: true),
        logger
    )
{
    ...
    {
        GattReadResult result = await _winDescriptor
            .ReadValueAsync()
    ...
    {
        GattWriteResult result = await _winDescriptor
            .WriteValueWithResultAsync(bytes.AsBuffer())
            .AsTask(cancellationToken)
            .ConfigureAwait(false);
