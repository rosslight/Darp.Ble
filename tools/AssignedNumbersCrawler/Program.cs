...
await using var adTypesWriter = new StreamWriter(
    File.Create($"{targetDirectory}/src/Darp.Ble/Data/AssignedNumbers/AdTypes.cs")
);
await adTypesWriter.WriteAsync(ReadAdTypes(numbersDirectory));
await using var companyIdentifierWriter = new StreamWriter(
    File.Create($"{targetDirectory}/src/Darp.Ble/Data/AssignedNumbers/CompanyIdentifiers.cs")
);
await companyIdentifierWriter.WriteAsync(ReadCompanyIdentifiers(numbersDirectory));
await using var coreVersionWriter = new StreamWriter(
    File.Create($"{targetDirectory}/src/Darp.Ble.Hci/AssignedNumbers/CoreVersion.cs")
);
await coreVersionWriter.WriteAsync(ReadCoreVersion(numbersDirectory));
return;

string ReadAdTypes(string inputDir) =>
    ReadFile<AdType, byte>(
...
string ReadCompanyIdentifiers(string inputDir) =>
    ReadFile<ValueNameObject, ushort>(
...
string ReadCoreVersion(string inputDir) =>
    ReadFile<ValueNameObject, byte>(
...
string ReadFile<T, TEnum>(string path, string nameSpace, string key, Func<T, int, string> func)
    where T : INameable
...
string FixNaming(string original, int i)
{
    if (original.Length == 0)
        throw new Exception("Value has an empty name");
...
string ToPascalCase(string original)
{
    ...
    var firstCharFollowedByUpperCasesOnly = new Regex(
        "(?<=[A-Z])[A-Z0-9]+$",
        RegexOptions.None,
    ...
    IEnumerable<string> pascalCase = invalidCharsRgx
