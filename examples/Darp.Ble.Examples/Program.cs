        ...
        .AddPeripheral(async device =>
        {
            ...
            await device.Broadcaster.StartAdvertisingAsync(
                ...
                data: AdvertisingData.Empty
                    .WithCompleteLocalName(device.Name!)
...
async Task AdvertiseApi(IBleDevice device)
{
    ...
    var measurementObservable = Observable.Interval(TimeSpan.FromSeconds(1))
    ...
    await device.Broadcaster.StartAdvertisingAsync(BleEventType.Connectable,
    ...
    await device.Broadcaster.StartAdvertisingAsync(type: BleEventType.AdvInd,
        peerAddress: new BleAddress((UInt48)0x1234),
        data: AdvertisingData.From([
            (AdTypes.CompleteLocalName, "Hello"u8.ToArray()),
        ]),
